...
exports.handler = async (event) => {
    ...
    try {
        ...
        const [wfRes, execRes] = await Promise.all([
            fetch(n8nUrl + '/api/v1/workflows/' + workflowId,
                { headers: { 'X-N8N-API-KEY': n8nKey } }),
            fetch(n8nUrl + '/api/v1/executions?workflowId=' + workflowId + '&limit=10',
                { headers: { 'X-N8N-API-KEY': n8nKey } })
        ]);
        ...
        console.error('agent-status.js error:', e);
