...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!variantId) return resp(400, { error: 'Plan not configured: ' + plan });
        const { data: profile } = await sb.from('profiles').select('email, full_name').eq('id', user.id).single();
        const res = await fetch('https://api.lemonsqueezy.com/v1/checkouts', { method: 'POST', headers: { 'Content-Type': 'application/vnd.api+json', 'Accept': 'application/vnd.api+json', 'Authorization': 'Bearer ' + process.env.LEMONSQUEEZY_API_KEY }, body: JSON.stringify({ data: { type: 'checkouts', attributes: { checkout_options: { embed: false }, checkout_data: { email: profile?.email || user.email, name: profile?.full_name || '', custom: { user_id: user.id } } }, relationships: { store: { data: { type: 'stores', id: String(process.env.LEMONSQUEEZY_STORE_ID) } }, variant: { data: { type: 'variants', id: String(variantId) } } } } }) });
