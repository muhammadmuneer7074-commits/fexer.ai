    ...
    try {
        ...
        if (user) {
            ...
            if (!credit.ok) {
                return resp(402, {
                    error: 'NO_CREDITS',
                    message: 'No credits remaining. Upgrade to continue.'
                });
            }
        }
        ...
        if (customInstructions && customInstructions.trim()) sys += ' ' + customInstructions.trim();
        if (deepThinking) sys += ' Think step by step before answering. Be thorough and accurate.';
        if (voiceMode) sys += ' You are in a live voice conversation. Keep replies short — 1-2 sentences unless asked for more.';
        ...
        if (deepSearch) {
            try {
                ...
                if (q.trim() && process.env.TAVILY_API_KEY) {
                    ...
                    if (tr.ok) {
                        ...
                        const ctx = (td.results || [])
                            .map((r, i) => `${i + 1}. ${r.title}\nURL: ${r.url}\n${r.content}`)
                            .join('\n\n');
                        if (ctx) sys += '\n\nLive web search results:\n' + ctx + '\n\nUse these to answer accurately and cite URLs.';
                    }
                }
            } catch (e) { /* search errors are non-fatal */ }
        }
        ...
        if (toolsEnabled) {
            reqBody.tools = [{
                ...
                function: {
                    name: 'generate_image',
                    description: 'Generate an image when the user asks for a picture or image.',
                    parameters: {
                        type: 'object',
        ...
        if (voiceMode) reqBody.max_tokens = 150;
        else if (deepThinking) reqBody.max_tokens = 2000;
        ...
        if (choice.finish_reason === 'tool_calls' && choice.message.tool_calls?.length > 0) {
            ...
            if (!imgRes.ok) {
                return resp(200, { choices: [{ message: { role: 'assistant', content: '⚠️ Image generation failed: ' + (imgData.error?.message || 'Try again.') } }] });
            }
